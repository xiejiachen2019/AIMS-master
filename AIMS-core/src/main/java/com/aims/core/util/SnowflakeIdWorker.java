...
/**
 * Snowflake算法是带有时间戳的全局唯一ID生成算法。它有一套固定的ID格式
 * <p>结构如下(每部分用-分开):<br>
 * 0 - 0000000000 0000000000 0000000000 0000000000 0 - 00000 - 00000 - 000000000000 <br>
 * 1 bit：因为二进制里第一个bit为如果是1，那么都是负数，但是我们生成的id都是正数，所以第一个bit统一都是0
 * 41 bit：表示的是时间戳，单位是毫秒。41 bit可以表示的数字多达2^41 - 1，也就是可以标识2 ^ 41 - 1个毫秒值，换算成年就是表示69年的时间
 * 10 bit：记录工作机器id，代表的是这个服务最多可以部署在2^10台机器上，也就是1024台机器。但是10 bit里5个bit代表机房id，5个bit代表机器id。意思就是最多代表2 ^ 5个机房（32个机房），每个机房里可以代表2 ^ 5个机器（32台机器）
...
public class SnowflakeIdWorker {
  ...
  // 机器ID所占位数
  protected long ID_BITS = 5L;
  // 数据中心ID所占位数
  protected long DATA_CENTER_ID_BITS = 5L;
  // 机器ID最大值31 (此移位算法可很快计算出n位二进制数所能表示的最大十进制数)
  private long MAX_ID;
  // 数据中心ID最大值31
  private long MAX_DATA_CENTER_ID;
  // Sequence所占位数
  protected long SEQUENCE_BITS = 12L;
  // 机器ID偏移量12
  private long ID_SHIFT_BITS;
  // 数据中心ID偏移量12+5=17
  private long DATA_CENTER_ID_SHIFT_BITS;
  // 时间戳的偏移量12+5+5=22
  ...
  private long dataCenterId;
    ...
    private void initFields() {
        ...
        // 机器ID偏移量12
        ID_SHIFT_BITS = SEQUENCE_BITS;
        // 数据中心ID偏移量12+5=17
        DATA_CENTER_ID_SHIFT_BITS = SEQUENCE_BITS + ID_BITS;
        // 时间戳的偏移量12+5+5=22
        TIMESTAMP_LEFT_SHIFT_BITS =
                SEQUENCE_BITS + ID_BITS + DATA_CENTER_ID_BITS;
        // Sequence掩码4095
        SEQUENCE_MASK = ~(-1L << SEQUENCE_BITS);
